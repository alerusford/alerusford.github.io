...
var manufacturer = $(element).find('.manufacturer').html();
var stock_status = $(element).find('.stock_status').html();
var model = $(element).find('.model').html();
          var price = $(element).find('.price').html();
          var cart = $(element).find('.button-group').html();
          var rating = $(element).find('.rating').html();
          ...
          if (name != null) {
              html += '<td class="name"><h5>' + name + '</h5></td>';
          }

if (manufacturer != null) {
    html += '<td class="manufacturer">' + manufacturer + '</td>';
}

if (model != null) {
    html += '<td class="model">' + model + '</td>';
}
if (stock_status != null) {
    html += '<td class="stock_status_price">' + stock_status + '</td>';
}
        ...
        //   } 
          if (price != null) {
              html += '<td class="price_list">' + price + '</td>';
          }
          if (cart != null) {
      ...
      $('.noparent').unwrap().unwrap();
          ...
          $('thead').remove();
          $('.noparent').parent().before('<thead><tr> <th></th><th style="text-align:left">Наименование</th><th>Бренд</th><th>Артикул</th><th>Наличие</th><th>Цена</th> <th></th> </tr></therd>');
	    } else {
	      $('.noparent').unwrap().unwrap();
	          ...
	          $('thead').remove();
	          $('.noparent').parent().before('<thead><tr> <th></th><th style="text-align:left">Наименование</th><th style="text-align:center">Бренд</th><th style="text-align:center">Артикул</th><th style="text-align:center">Наличие</th><th style="text-align:center">Цена</th> <th></th> </tr></therd>');
	    }        
