...
class ModelToolOcextSmartSearch extends Model {
        ...
        foreach ($words_array as $position=>$keyword) {
            ...
            if(isset($rang['product'][$keyword])){
                if(isset($weight[$position])){
                    $total_product_rang += $weight[$position]*$rang['product'][$keyword];
                }
                $total_product_rang += $rang['product'][$keyword];
            }
            
            if ($this->getStristrIntoIndex($rang['product'], $keyword)) {
                if(isset($weight[$position])){
                    $total_product_rang += $weight[$position]*$rang['product'][ $this->getStristrIntoIndex($rang['product'], $keyword) ];
                }
            ...
            if($this->config->get('ocext_smart_search_attribute')){
                if(isset($rang['attribute'][$keyword])){
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['attribute'][$keyword];
                    }
                    $total_product_rang += $rang['attribute'][$keyword];
                }
                if ($this->getStristrIntoIndex($rang['attribute'], $keyword)) {
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['attribute'][ $this->getStristrIntoIndex($rang['attribute'], $keyword) ];
                    }
            ...
            if($this->config->get('ocext_smart_search_option')){
                if(isset($rang['options'][$keyword])){
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['options'][$keyword];
                    }
                    $total_product_rang += $rang['options'][$keyword];
                }
                if ($this->getStristrIntoIndex($rang['options'], $keyword)) {
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['options'][ $this->getStristrIntoIndex($rang['options'], $keyword) ];
                    }
            ...
            if($this->config->get('ocext_smart_search_category')){
                if(isset($rang['category'][$keyword])){
                    ...
                    $total_product_rang += $rang['category'][$keyword]*$rang['category'][$keyword];
                }
                if ($this->getStristrIntoIndex($rang['category'], $keyword)) {
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['category'][ $this->getStristrIntoIndex($rang['category'], $keyword) ];
                    }
            ...
            if($this->config->get('ocext_smart_search_manufacturer')){
                if(isset($rang['manufacturer'][$keyword])){
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['manufacturer'][$keyword];
                    }
                    $total_product_rang += $rang['manufacturer'][$keyword];
                }
                if ($this->getStristrIntoIndex($rang['manufacturer'], $keyword)) {
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['manufacturer'][ $this->getStristrIntoIndex($rang['manufacturer'], $keyword) ];
                    }
            ...
            if($this->config->get('ocext_smart_search_review')){
                if(isset($rang['review'][$keyword])){
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['review'][$keyword];
                    }
                    $total_product_rang += $rang['review'][$keyword];
                }
                if ($this->getStristrIntoIndex($rang['review'], $keyword)) {
                    if(isset($weight[$position])){
                        $total_product_rang += $weight[$position]*$rang['review'][ $this->getStristrIntoIndex($rang['review'], $keyword) ];
                    }
    ...
    public function cleanPretextRu(&$words,$cleanHTML=FALSE,$ignorDotAndComa=FALSE){
        ...
        $result = '';
        $pretexts = array('the','a','on','у',',','.','-', 'о', 'в', 'с', 'к', 'за', 'до', 'на','обо','об','при', 'по', 'из', 'от', 'над', 'под', 'про', 'без', 'для', 'ради', 'через','все','всё','и','a','но');
