...
class ModelExtensionExchange1c extends Model {
	...
	private function prepareQueryProduct($data, $mode = 'set') {
		...
		if (isset($data['hide']))
	 		$sql[] = $mode == 'set'		? "`hide` = '" .				(int)$data['hide'] . "'"						: "`hide`";
		if (isset($data['warranty']))
	 		$sql[] = $mode == 'set'		? "`warranty` = '" .				(int)$data['warranty'] . "'"						: "`warranty`";
	...
	private function parseRequisite($xml, &$data) {
		...
		foreach ($xml->ЗначениеРеквизита as $requisite) {
			...
			switch ($name){
				...
				break;
				case 'Ячейка':
					$data['location'] = htmlspecialchars($value);
					$this->log("> Реквизит: " . $name . " = " . $data['location'], 2);
				break;
				case 'ISBN':
					$data['isbn'] = htmlspecialchars($value);
					$this->log("> Реквизит: " . $name . " = " . $data['isbn'], 2);
				break;
				case 'ИсключитьИзПрайсЛистов':
					$data['hide'] = (string)$value == 'false' ? 0 : 1;
					$this->log("> Реквизит: " . $name . " = " . $data['hide'], 2);
				break;
				case 'ГарантийныйСрок':
					$data['warranty'] = (int)$value;
					$this->log("> Реквизит: " . $name . " = " . $data['warranty'], 2);
				break;
			} // switch
		} // foreach()
