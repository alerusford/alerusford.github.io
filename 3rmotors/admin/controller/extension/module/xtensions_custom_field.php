...
class ControllerExtensionModuleXtensionsCustomField extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->xtensions_checkout->redirect($this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		array('text'=>$this->language->get('heading_title'),'href'=> $this->url->link($this->config->get('xtensions_custom_fields_path'),'token=' . $this->session->data['token'], 'SSL'),'separator' => ' :: ')
		);

		$this->data['add'] = $this->url->link($this->config->get('xtensions_custom_fields_path').'/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link($this->config->get('xtensions_custom_fields_path').'/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$this->data['custom_fields'][] = array(
				...
				'edit'            => $this->url->link($this->config->get('xtensions_custom_fields_path').'/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $result['custom_field_id'] . $url, 'SSL')
			);
		}
		...
		$this->data['sort_name'] = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . '&sort=cfd.name' . $url, 'SSL');
		$this->data['sort_location'] = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . '&sort=cf.location' . $url, 'SSL');
		$this->data['sort_type'] = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . '&sort=cf.type' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . '&sort=cf.status' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . '&sort=cf.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template =  $this->config->get('xtensions_custom_fields_view_path').'/xcustom_field_list.tpl';
	...
	protected function getForm() {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' :: '
			);

			if (!isset($this->request->get['custom_field_id'])) {
				$this->data['action'] = $this->url->link($this->config->get('xtensions_custom_fields_path').'/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
			} else {
				$this->data['action'] = $this->url->link($this->config->get('xtensions_custom_fields_path').'/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, 'SSL');
			}

			$this->data['cancel'] = $this->url->link($this->config->get('xtensions_custom_fields_path'), 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			$this->data['best_checkout'] = $this->url->link($this->config->get('xtensions_admin_xcheckout_path'),'token=' . $this->session->data['token']);
			$this->load->model('customer/customer_group');
			$this->data['customer_groups'] = $this->model_customer_customer_group->getCustomerGroups();			
			$this->template =  $this->config->get('xtensions_custom_fields_view_path').'/xcustom_field_form.tpl';
			$this->data['header'] = $this->load->controller('common/header');
			$this->data['column_left'] = $this->load->controller('common/column_left');
			$this->data['footer'] = $this->load->controller('common/footer');
			$this->response->setOutput($this->load->view($this->template, $this->data));
