...
class ControllerExtensionModuleMegamenuvh extends Controller
{
	private $error = array();
	...
	{
		$this->load->model('extension/module/megamenuvh');
		$this->model_extension_module_megamenuvh->installDB();
	}
	...
	private function licenseValidation()
	{
		...
		if ($this->aleksey_18 && $this->aleksey_39() && $this->isWindows() && !file_exists($this->aleksey_5 . $this->aleksey_24)) {
			...
			$this->activation();
			$this->response->redirect($this->url->link('extension/module/megamenuvh/activation', 'token=' . $this->session->data['token'], true));
		}
	...
	public function index()
	{
		...
		if ($license_key->num_rows <= 0) {
			$this->db->query('INSERT INTO ' . DB_PREFIX . 'megamenu_key (`key`, `license_key`) VALUES(\'local_key\', \'\');');
		}

		$this->licenseValidation();
		$this->load->language('extension/module/megamenuvh');
		$this->document->addStyle('view/stylesheet/style_megamenu.css');
		$this->document->setTitle(strip_tags($this->language->get('heading_title')));
		$this->load->model('setting/setting');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('megamenu_setting', $this->request->post);
	...
	public function getList()
	{
		...
		$data['breadcrumbs'][] = array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'], 'SSL'));
		$data['lang_id'] = $this->config->get('config_language_id');
		$data['add'] = $this->url->link('extension/module/megamenuvh/addItem', 'token=' . $this->session->data['token'], 'SSL');
		$data['action'] = $this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=module', true);
		$data['delete'] = $this->url->link('extension/module/megamenuvh/deleteItem', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['sort_menu_type'] = $this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'] . '&sort=menu_type' . $url, true);
		$data['sort_status'] = $this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
		$data['sort_menu'] = $this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'] . '&sort=sort_menu' . $url, true);
		$url = '';
		...
		$this->response->setOutput($this->load->view('extension/module/megamenuvh', $data));
	}
	...
	public function getForm()
	{
		...
		if (!isset($this->request->get['megamenu_id'])) {
			$data['breadcrumbs'][] = array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/module/megamenuvh/addItem', 'token=' . $this->session->data['token'] . $url, 'SSL'), 'separator' => ' :: ');
		}
		else {
			$data['breadcrumbs'][] = array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/module/megamenuvh/editItem', 'token=' . $this->session->data['token'] . '&megamenu_id=' . $this->request->get['megamenu_id'] . $url, 'SSL'), 'separator' => ' :: ');
		}

		$data['token'] = $this->session->data['token'];

		if (!isset($this->request->get['megamenu_id'])) {
			$data['action'] = $this->url->link('extension/module/megamenuvh/addItem', 'token=' . $this->session->data['token'] . $url, true);
		}
		else {
			$data['action'] = $this->url->link('extension/module/megamenuvh/editItem', 'token=' . $this->session->data['token'] . '&megamenu_id=' . $this->request->get['megamenu_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'] . $url, true);
		...
		if (isset($this->request->get['megamenu_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$menuvh_info = $this->model_extension_module_megamenuvh->getItem($this->request->get['megamenu_id']);
		}
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		$this->response->setOutput($this->load->view('extension/module/megamenu/megamenuvh_form', $data));
	}
	...
	public function saveTypeMenu()
	{
		...
		if (isset($this->request->get['megamenu_id'])) {
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormTypeMenu()) {
				$this->model_extension_module_megamenuvh->saveTypeMenu($this->request->get['megamenu_id'], $this->request->post['menuvh']);
	...
	public function saveLinkMenu()
	{
		...
		if (isset($this->request->get['megamenu_id'])) {
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePopup()) {
				$this->model_extension_module_megamenuvh->saveLinkMenu($this->request->get['megamenu_id'], $this->request->post['menuvh']);
				$json['success'] = $this->language->get('text_success');
				return $this->response->setOutput(json_encode($json));
			}
	...
	public function saveStickerMenu()
	{
		...
		if (isset($this->request->get['megamenu_id'])) {
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePopup()) {
				$this->model_extension_module_megamenuvh->saveStickerMenu($this->request->get['megamenu_id'], $this->request->post['menuvh']);
				$json['success'] = $this->language->get('text_success');
				return $this->response->setOutput(json_encode($json));
			}
	...
	public function saveNameItem()
	{
		...
		if (isset($this->request->get['megamenu_id'])) {
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePopupNameItem()) {
				$this->model_extension_module_megamenuvh->saveNameItem($this->request->get['megamenu_id'], $this->request->post['menuvh']);
				$json['success'] = $this->language->get('text_success');
				return $this->response->setOutput(json_encode($json));
			}
	...
	public function addItem()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_module_megamenuvh->addItem($this->request->post['menuvh']);
			$this->session->data['success'] = $this->language->get('text_success');
			$url = '';
			...
			$this->response->redirect($this->url->link('extension/module/megamenuvh', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function editItem()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_module_megamenuvh->editItem($this->request->get['megamenu_id'], $this->request->post['menuvh']);
			$this->session->data['success'] = $this->language->get('text_success');
			$url = '';
	...
	public function deleteItem()
	{
		...
		if (isset($this->request->post['selected']) && $this->validateDeleteItem()) {
			foreach ($this->request->post['selected'] as $megamenu_id) {
				$this->model_extension_module_megamenuvh->deleteItem($megamenu_id);
			}
	...
	public function changeStatus()
	{
		...
		if (isset($this->request->get['object_id']) && $this->validatePopup()) {
			...
			if (count($get_request) == 2) {
				...
				$result = $this->model_extension_module_megamenuvh->getItem($tabs_ns_id);

				if ($result[$column_name]) {
					$this->model_extension_module_megamenuvh->changeStatus($tabs_ns_id, 0);
				}
				else {
					$this->model_extension_module_megamenuvh->changeStatus($tabs_ns_id, 1);
				}
	...
	{
		if (!$this->user->hasPermission('modify', 'extension/module/megamenuvh')) {
	...
	public function autocategoryadd()
	{
		...
		if (isset($this->request->get['autocategoryadd']) && ($this->request->get['autocategoryadd'] != '') && $this->validatePopup()) {
			...
			$this->load->model('extension/module/megamenuvh');
			$this->model_extension_module_megamenuvh->createsItem($data['config_menu_items_new']);
			$json['success'] = $this->language->get('text_success');
			$this->session->data['success'] = $this->language->get('text_success');
		}
		else {
	...
	public function activation()
	{
		...
		$data['breadcrumbs'][] = array('text' => $this->language->get('text_home'), 'href' => $this->url->link('common/home', 'token=' . $this->session->data['token'], 'SSL'), 'separator' => false);
		$url = '';
		$data['breadcrumbs'][] = array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/module/megamenuvh/activation', 'token=' . $this->session->data['token'] . $url, 'SSL'), 'separator' => ' :: ');
		$this->load->model('localisation/language');
		$data['languages'] = $this->model_localisation_language->getLanguages();
		$data['token'] = $this->session->data['token'];
		$data['action'] = $this->url->link('extension/module/megamenuvh/activation', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=module', true);
		$data['heading_title_activation'] = $this->language->get('heading_title_activation');
